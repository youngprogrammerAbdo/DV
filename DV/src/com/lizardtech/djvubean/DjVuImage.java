...
public class DjVuImage
{
  ...
  /** A constant indicating the color black. */
...
//      final Rectangle scaled = new Rectangle();
      ...
      for(int pos = 0; pos < highlightList.size();)
      {
        ...
        if (markup==2)
       {g.drawRect(xmin, ymin+(ymax-ymin), xmax-xmin, 0);
       g.drawRect(xmin, ymin+(ymax-ymin)+1, xmax-xmin, 0);
       g.drawRect(xmin, ymin+(ymax-ymin)+2, xmax-xmin, 0);}

        // under line 
       
        if (markup==3)
        { g.drawRect(xmin, ymin+((ymax-ymin)/2 ), xmax-xmin, 0);
        g.drawRect(xmin, ymin+((ymax-ymin)/2 )+1, xmax-xmin, 0);
        g.drawRect(xmin, ymin+((ymax-ymin)/2 )+2, xmax-xmin, 0);}
      }
    }
  }
