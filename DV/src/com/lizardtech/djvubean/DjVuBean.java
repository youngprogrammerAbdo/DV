...
import java.net.URL;
import java.util.*;
import static com.lizardtech.djvubean.RibbonMenu.DjvuComponents.drawing;
import static com.lizardtech.djvubean.RibbonMenu.DjvuComponents.typewriter;
import static com.lizardtech.djvubean.Markmode.xx;
import static com.lizardtech.djvubean.Markmode.yy;
import static com.lizardtech.djvubean.Markmode.a;
import static com.lizardtech.djvubean.Markmode.b;

import com.lizardtech.djvubean.anno.AnnoManager;
...
public class DjVuBean extends Panel implements PropertyChangeListener, DjVuInterface, java.io.Serializable, Runnable {
  //~ Static fields/initializers ---------------------------------------------
     ...
     */
    public static final String ZOOM800 = "800%";
    /**
     * String used for selecting 600% zoom.
     */
    public static final String ZOOM600 = "600%";
    /**
     * String used for selecting 400% zoom.
     */
    public static final String ZOOM400 = "400%";
    /**-
     * String used for selecting 200% zoom.
     */
    public static final String ZOOM200 = "200%";
     ...
     */
    public static final String ZOOM_FIT_WIDTH = "146%";

    /**
     * String used for selecting fit page zoom.
     */
    public static final String ZOOM_FIT_PAGE = "53%";
...
//    repaint(50L); // strictly speaking this shouldn't be needed.
    ...
    public void paintComponent(Graphics g) {
        ...
        if (markup == 1) {
            ...
            image.fillRect(g, XOR_HILITE, 0, djvubean.getSelect().x, djvubean.getSelect().y, djvubean.getSelect().width, djvubean.getSelect().height, 1);
            g.setXORMode(Color.WHITE);

        }

        if (markup == 2) {
            image.fillRect(g, XOR_HILITE, 50, djvubean.getSelect().x, djvubean.getSelect().y, djvubean.getSelect().width, djvubean.getSelect().height, 2);
            g.setXORMode(Color.white);
        ...
        if (isShowing()) {
            ...
            try {
                ...
                if (!isDecoding()) {
                   ...
                   // for highlights   
//                    if (markup==1){
//  image.fillRect(g,XOR_HILITE,50,djvubean.getSelect().x ,djvubean.getSelect().y,djvubean.getSelect().width,djvubean.getSelect().height,1);
//                   
//                    }
                    // underline  
//                    if (markup==2)
...
//                    }
                    if (markup == 3) {
                       // strikethrough
                        ...
                        image.fillRect(g, XOR_HILITE, 50, djvubean.getSelect().x, djvubean.getSelect().y, djvubean.getSelect().width, djvubean.getSelect().height, 3);
                    }
                    // serach
                    if (markup == 5) {
                    ...
                    if (drawing == 3) {
                       ...
                       // another solution
//                    g.drawLine(xx, yy, a,b);
//                    g.drawLine(xx+1, yy+1, a+1,b+1);
//                    g.drawLine(xx+2, yy+2, a+2,b+2);
//                    g.drawLine(xx+3, yy+3, a+3,b+3);
                        image.fillRect(g, XOR_HILITE, 50, djvubean.getSelect().x, djvubean.getSelect().y, djvubean.getSelect().width, djvubean.getSelect().height, 4);
                    }
                }
...
//                           g.drawString(text,djvubean.getSelect().x , djvubean.getSelect().y);
//          
                    new PopupDemo1(100, 200);
                    PopupDemo1 a = new PopupDemo1(100, 200);
                    String text = a.txt;
                    g.drawString(text, djvubean.getSelect().x, djvubean.getSelect().y);
                ...
                if (drawing == 2) {
                    if ((select.width > 1) && (select.height > 1)) {
                        if (useXOR) {
                            g.setXORMode(DjVuImage.WHITE);
                            g.drawOval(select.x, select.y, select.width, select.height);
                ...
                if (drawing == 4) {
                    if ((select.width > 1) && (select.height > 1)) {
                        if (useXOR) {
                            g.setXORMode(DjVuImage.WHITE);
                            g.drawRoundRect(select.x, select.y, select.width, select.height, 100, 30);
...
//                    g.drawLine(200, 100, 400, 90);
//                     g.drawLine( 400, 90,500,120);
                }
...
//                  AnnoManager a=new AnnoManager(this);
//                  
//                  a.switchColor(g, Rect.XOR_HILITE);
                }
                ...
                if (drawing == 8) {
                    ...
                    // draw arrow 
                    AnnoManager a = new AnnoManager(this);
                    a.drawLine(g, select.x, select.y, xx, yy, 5, true);
                ...
                if (drawing == 9) {
                    ...
                    // area highlights
                    image.fillRect(g, XOR_HILITE, 50, djvubean.getSelect().x, djvubean.getSelect().y, djvubean.getSelect().width, djvubean.getSelect().height, 5);
                ...
                if (popup_note == 1) {
                    ...
                    URL iconURL = getClass().getResource("/images/Comment48.png");
                    ImageIcon icon = new ImageIcon(iconURL);
                    g.drawImage(icon.getImage(), xx - 10, yy - 25, 20, 20, null);
...
class PopupDemo1 extends JDialog implements ActionListener {

    public String txt;

    public PopupDemo1(int x, int y) {
        ...
        Date date = new Date();
        // create dialog
        JDialog dialog = new JDialog(this, "insert text");
        ...
        // header 
        JTextArea title = new JTextArea(1, 10);
        title.setFont(new Font("arial", 0, 12));
        title.setText(hostname + "    " + dateFormat.format(date));
        title.setEditable(false);
        // content 
        JTextArea note = new JTextArea();
        note.setFont(new Font("arial", 0, 13));
        dialog.add(title, BorderLayout.PAGE_START);
        dialog.add(note, BorderLayout.CENTER);
        note.setBackground(Color.gray);
        title.setBackground(Color.MAGENTA);
        dialog.setBackground(Color.MAGENTA);
        // border color 
        dialog.getRootPane().setBorder(BorderFactory.createLineBorder(Color.MAGENTA, 2));
        dialog.setModalityType(Dialog.ModalityType.APPLICATION_MODAL);
        dialog.setSize(200, 140);
        dialog.setLocation(1150, y + 60);
