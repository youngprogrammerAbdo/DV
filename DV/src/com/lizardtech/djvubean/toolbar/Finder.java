...
import static com.lizardtech.djvubean.RibbonMenu.RibbonMenuCreation.AreaSearch;
  ...
  implements ActionListener, PropertyChangeListener
{
 ...
 // public static Component searchField;
  ...
  //private static  DjVuBean djvuBean = null;
  ...
  public Finder(final DjVuBean bean)
  {
   // AreaSearch=null;
    if(classTextField != TextField.class)
    {
      try
      {
        AreaSearch= (Component)classTextField.newInstance();
      }
      catch(final Throwable exp) {}
    }
//    if(AreaSearch == null)
//    {
//      AreaSearch = new JTextField();
//    }
    ...
    if(TextField.class.isAssignableFrom(AreaSearch.getClass()))
    {
      ((TextField)AreaSearch).addActionListener(this);
    }
    else
    {
      final Object[] args = {this};
      DjVuObject.invoke(addActionListenerMethod,AreaSearch, args);
    }
    ...
    else
    {
      if(TextField.class.isAssignableFrom(AreaSearch.getClass()))
      {
        ((TextField)AreaSearch).setText(text);
      }
      else
      {
        final Object[] args = {text};
        DjVuObject.invoke(setTextMethod,AreaSearch, args);
      }      
    ...
    if(firstTime)
    {
      ...
      setVisible(
        DjVuBean.stringToBoolean(
          ...
          false));
      final String text=(TextField.class.isAssignableFrom(AreaSearch.getClass()))?
        ((TextField)AreaSearch).getText():  
        (String)DjVuObject.invoke(getTextMethod,AreaSearch, null);
      if((text != null)&&(text.trim().length() != 0))
      {
  ...
  public static boolean searchDocument(final boolean forward)
  {
      
    final  String text=
      (TextField.class.isAssignableFrom(AreaSearch.getClass()))?
        ((TextField)AreaSearch).getText():  
        (String)DjVuObject.invoke(getTextMethod,AreaSearch, null);
  ...
  //
   ...
   public static boolean searchDocument(final boolean forward,String x)
  {
...
//        (String)DjVuObject.invoke(getTextMethod,AreaSearch, null);
   ...
   *         found.
   */
  public static boolean searchPage(final boolean forward)
  {
    final  String text=
      (TextField.class.isAssignableFrom(AreaSearch.getClass()))?
        ((TextField)AreaSearch).getText():  
      ...
      && ((width < 750) || (targetWidth < 750)))
    {
      if((width < 480) || (targetWidth < 480))
      {
        ...
        if(TextField.class.isAssignableFrom(AreaSearch.getClass()))
        {
          for(int i = 30; i > 5; i--)
          {
            ((TextField)AreaSearch).setColumns(i);  
            if(
              AreaSearch.getPreferredSize().width <= wmax)
            {
              break;
            }
          }
        ...
        else
        {
          for(int i = 30; i > 5; i--)
          {
            ...
            DjVuObject.invoke(setColumnsMethod,AreaSearch, args);
            if(
              AreaSearch.getPreferredSize().width <= wmax)
            {
              break;
